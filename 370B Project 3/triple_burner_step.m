function [finalState, Tpeak, mdot_mix] = burner(gas, air, water, mdot_air, mdot_w, P1_air, Tmax,Burner_Pressure_Ratio)

hf = enthalpy_mass(gas);
sf = entropy_mass(gas);
ha = enthalpy_mass(air);
sa = entropy_mass(air);
hw = enthalpy_mass(water);
sw = entropy_mass(water);
...
for mdot_fuel=dfuel:dfuel:mdot_air
    mdot_mix = mdot_air + mdot_fuel + mdot_w;
    M_fuel = meanMolecularWeight(gas);
    
    % Mix the air and fuel adiabatically.
    Pm = P1_air;
    hm = (mdot_fuel*hf + mdot_air*ha + mdot_w*hw)/mdot_mix;
    ...
    xmix(iH2O)  = xmix(iH20) + 1*mdot_w/M_water;
    xmix = xmix/sum(xmix);
